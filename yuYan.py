...
def down_xiangQing_html(row,mulu,jianJie,lianjie,count,worksheet,folderName):
    output_dir = mulu
    ...
    list_tp1 = tree.xpath('//*[@id="x-section-1"]/div/div[1]/h1/span/text()')
    if len(list_tp1) == 0 :
        list_tp1 = tree.xpath('//*[@id="x-section-1"]/div/div[1]/h4/span//text()')

    list_tp2 = tree.xpath('//*[@id="x-section-1"]/div/div[1]/div//text()')
    list_publish = tree.xpath('//*[@id="x-legacy-panel-1"]/table/tbody/tr[1]/td[2]//text()')
    if len(list_publish) == 0 :
        list_publish = tree.xpath('//*[@id="x-legacy-panel-1"]/table/tbody/tr[1]/td[2]/a/text()')
        if len(list_publish) ==0 :
            list_publish = tree.xpath('//*[@id="x-legacy-panel-1"]/table/thead/tr[1]/td[2]//text()')
            if len(list_publish) == 0:
                list_publish=[""]

    list_author = tree.xpath('//*[@id="x-legacy-panel-1"]/table/tbody/tr[2]/td[2]/a/text()')
    ...
    list_yuYan = tree.xpath('//*[@id="x-legacy-panel-1"]/table/tbody/tr[3]/td[2]/a/text()')
    if len(list_yuYan) == 0 :
    ...
    worksheet.write(row, 1, folderName)
    worksheet.write(row, 2, title)
    worksheet.write(row, 3, str(list_yuYan[0]))
    worksheet.write(row, 4, str(list_author[0]))
    worksheet.write(row, 5, str(list_publish[0]))
    worksheet.write(row, 6, jianJie)
    worksheet.write(row, 7, lianjie)


def handleData(output_dir,link,folderName):
   ...
   # down_load(output_dir + folderName, c1, "html")
    ...
    if len(list_zip) == 1:
        ...
        list_jianJie = tree.xpath(
            '//*[@id="x-site"]/main/div[3]/div/div/div/div/div/div/article/div/p[3]/text()')
        if len(list_jianJie) == 0:
            list_jianJie = tree.xpath(
                '//*[@id="x-site"]/main/div[3]/div/div/div/div/div/div/article/div/p[2]/text()')
        oneAbstract = str(list_jianJie[0])
        oneLink = str(
            tree.xpath('//*[@id="x-site"]/main/div[3]/div/div/div/div/div/div/article/div/a/@href')[0])
        down_xiangQing_html(row, mulu2, oneAbstract, oneLink, count, worksheet,folderName)
        row += 1
        ...
        while count < 100:
            ...
            down_xiangQing_html(row, mulu2, abstractOne, linkOne, count, worksheet,folderName)
...
if __name__ == '__main__':
    ...
    parser = etree.HTMLParser(encoding="utf-8")
    tree = etree.parse(
        "D:\pythonProject\Dharmaebooks\Authors • dharmaebooks.org • Library of Buddhist and Tibetan Ebooks.html",
        parser=parser)
    ...
    list_link2.remove(
        "/cdn-cgi/l/email-protection#")
    ...
    list_link2.remove(
        "/cdn-cgi/l/email-protection#")
    list_link2.remove("https://dharmaebooks.org/category/teachings/")
    print(list_link2)

    # 写入excel
    workbook = xlsxwriter.Workbook('dharmaebooks.xlsx')  # 创建一个excel文件
    worksheet = workbook.add_worksheet('目录')  # 在文件中创建一个名为语言的sheet,不加名字默认为sheet1




    worksheet.write(0, 0, "ID")
    worksheet.write(0, 1, "catalogue")
    worksheet.write(0, 2, "title")
    worksheet.write(0, 3, "languages")
    worksheet.write(0, 4, "author")
    worksheet.write(0, 5, "publisher")
    ...
    for link in list_link2:
        if str(link).startswith("https://dharmaebooks.org/tag"):
            ...
            handleData(output_dir,link,folderName)
            for i in range(2,11):
                ...
                try:
                    ...
                    handleData(output_dir2, link2,folderName)
